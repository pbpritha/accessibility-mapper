...
That's the entire real-time mechanism — no WebSocket server, no custom pub/sub to build.
...
| Config | `.env` with `VITE_SUPABASE_URL`, `VITE_SUPABASE_ANON_KEY`, loaded via Vite's `import.meta.env` |
...
- No auth/accounts, no login screens.
- No moderation queue or content filtering on photos.
- No push notifications.
- No routing/navigation engine — pins are informational only.
